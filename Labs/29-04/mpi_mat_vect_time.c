...
int mat_sizes[] = {1024, 2048, 4096, 8192, 16384};

void Check_for_error(int local_ok, char fname[], char message[], 
      MPI_Comm comm);
void Get_dims(int size, int* m_p, int* local_m_p, int* n_p, int* local_n_p,
...
int main(void) {
   ...
   for(int i = 0; i < 5; i++){
	   Get_dims(mat_sizes[i], &m, &local_m, &n, &local_n, my_rank, comm_sz, comm);
